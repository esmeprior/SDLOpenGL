...
Shader::Shader(const std::string &_vert, const std::string &_frag)
{
 std::ifstream shaderSource(_vert.c_str());
  ...
  //copy and pasted code for fragment
  ...
  shaderSource.open(_frag.c_str());
   ...
   //need to link shaders together in a programme - vertex and fragment combined into colour
...
void Shader::printInfoLog(GLuint _id, DebugMode _mode)
{
  ...
  if(infoLogLenth > 0)
    {
      std::unique_ptr<char []>infoLog(new char[infoLogLenth]);
      glGetShaderInfoLog(_id,infoLogLenth,&charsWritten,infoLog.get());
      std::cerr<<"Log \n"<<infoLog.get()<<"\n";
      GLenum mode;
      ...
      glGetShaderiv(_id,mode,&infoLogLenth);
      if(infoLogLenth == GL_FALSE)
      {
